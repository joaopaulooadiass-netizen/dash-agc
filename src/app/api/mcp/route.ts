...
function criarServidor() {
  const server = new McpServer({ name: 'dashboard-copy-que-vende', version: '2.0.0' })
...
function naoAutorizado(): Response {
  return new Response(JSON.stringify({ error: 'unauthorized', message: 'Token Bearer ausente ou inválido.' }), {
    status: 401,
    headers: { 'content-type': 'application/json', 'www-authenticate': 'Bearer realm="dashboard-copy-que-vende"' },
  })
}
