                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                  {metricas.filter(m => m.grupo === grupo).map(m => (
                    <button key={m.key} onClick={() => { onSelect(m.key); setAberto(false) }}
                      style={{ padding: '0.26rem 0.55rem', fontFamily: 'var(--font-body)', fontSize: '0.72rem', borderRadius: '4px', border: '1px solid', borderColor: atual.key === m.key ? 'var(--color-ponto-conversao)' : 'var(--color-border-subtle)', cursor: 'pointer', backgroundColor: atual.key === m.key ? 'rgba(204,155,73,0.15)' : 'var(--color-bg-card)', color: atual.key === m.key ? 'var(--color-ponto-conversao)' : 'var(--color-text-secondary)', fontWeight: atual.key === m.key ? 700 : 400 }}>
                      {m.label}
                    </button>
                  ))}
                </div>
      ...
      <div style={{ backgroundColor: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
        ...
        <ResponsiveContainer width="100%" height={380}>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 30, left: 10 }}>
            ...
            {mediaX > 0 && <ReferenceLine x={mediaX} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 4" />}
            {mediaY > 0 && <ReferenceLine y={mediaY} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 4" />}
