...
const TIPOS: Record<TipoLista, { label: string; cor: string; desc: string }> = {
  meta_ads:    { label: 'Meta Ads',    cor: '#3B82F6', desc: 'Export do Gerenciador de Anúncios' },
  leads_utm:   { label: 'Leads + UTM', cor: '#8B5CF6', desc: 'Leads com parâmetros UTM' },
  compradores: { label: 'Compradores', cor: '#22C55E', desc: 'Lista de quem comprou' },
  pesquisa:    { label: 'Pesquisa',    cor: '#F59E0B', desc: 'Respostas de pesquisa' },
  desistencia: { label: 'Desistência', cor: '#EF4444', desc: 'Pesquisa de quem não comprou' },
  matricula:   { label: 'Matrícula',   cor: '#10B981', desc: 'Dados dos alunos matriculados' },
  custom:      { label: 'Customizado', cor: '#94A3B8', desc: 'Qualquer outro tipo de lista' },
}
...
function derivarStatus(presenca: Record<string, boolean>, listas: ListaImportada[]): { label: string; cor: string } {
  if (listas.some(l => l.tipo === 'compradores' && presenca[l.id])) return { label: 'Comprador',  cor: '#22C55E' }
  if (listas.some(l => l.tipo === 'desistencia' && presenca[l.id])) return { label: 'Desistente', cor: '#EF4444' }
  return { label: 'Lead', cor: '#94A3B8' }
}
      ...
      <div style={{ width: '256px', flexShrink: 0, backgroundColor: 'var(--color-bg-secondary)', borderRight: '1px solid var(--color-border-subtle)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0.75rem 0.75rem 0' }}>
          {listas.map(l => {
            ...
            return (
              <div key={l.id} style={{ padding: '0.55rem 0.65rem', marginBottom: '0.3rem', borderRadius: 'var(--radius-md)', backgroundColor: previewing ? 'rgba(204,155,73,0.06)' : 'var(--color-bg-card)', border: `1px solid ${previewing ? 'var(--color-ponto-conversao)' : checked ? `${cfg.cor}55` : 'var(--color-border-subtle)'}` }}>
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0', borderBottom: '1px solid var(--color-border-subtle)', flexShrink: 0 }}>
                  {[
                    ...
                    { label: 'Compradores',   val: sumario.compradores, cor: '#22C55E' },
                    { label: 'Desistentes',   val: sumario.desistentes, cor: '#EF4444' },
                    { label: 'Só Lead',       val: sumario.leads,       cor: '#94A3B8' },
                  ].map(({ label, val, cor }) => (
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ borderBottom: '1px solid var(--color-border-subtle)', flexShrink: 0 }}>
              ...
              <div style={{ padding: '0.6rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <button onClick={() => setConfigAberta(v => !v)} style={{ padding: '0.28rem 0.65rem', backgroundColor: configAberta ? 'rgba(204,155,73,0.1)' : 'var(--color-bg-card)', border: `1px solid ${configAberta ? 'var(--color-ponto-conversao)' : 'var(--color-border-subtle)'}`, borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: configAberta ? 'var(--color-ponto-conversao)' : 'var(--color-text-muted)', cursor: 'pointer' }}>
                  ⚙ Configurar colunas
                </button>
                {configCPA && (
                            ...
                            const campoMeta = (label: string, key: 'colMetaCamp' | 'colMetaConjunto' | 'colMetaAnuncio') => {
                              ...
                              return (
                                <div>
                                  ...
                                  <select value={cfgAtiva[key]} onChange={e => setConfigCPA({ ...cfgAtiva, [key]: e.target.value })} style={{ width: '100%', padding: '0.35rem 0.5rem', backgroundColor: 'var(--color-bg-card)', border: `1.5px solid ${usado ? 'var(--color-ponto-conversao)' : cfgAtiva[key] ? '#22C55E' : 'var(--color-border-subtle)'}`, borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: 'var(--color-text-primary)', cursor: 'pointer' }}>
                                    <option value="">— opcional —</option>
                                    {l.headers.map(h => <option key={h} value={h}>{h}</option>)}
                                  </select>
                                </div>
