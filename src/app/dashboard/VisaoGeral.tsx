...
const COR_ETAPA_FALLBACK = '#94A3B8'
...
const METRICAS_GRAFICO = [
  { metricKey: 'investimento', key: 'Investimento', label: 'Investimento', cor: '#60A5FA', eixo: 'R$' },
  { metricKey: 'receita',      key: 'Receita',      label: 'Receita',      cor: '#22C55E', eixo: 'R$' },
  { metricKey: 'compras',      key: 'Compras',      label: 'Compras',      cor: '#F59E0B', eixo: 'n'  },
]
...
function GraficoCard({ dias, dados }: { dias: number; dados: Record<string, number | string>[] }) {
  ...
  const gradients: Record<MetricaGraficoKey, string> = { Investimento: '#60A5FA', Receita: '#22C55E', Compras: '#F59E0B' }
    ...
    <div style={{ backgroundColor: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-lg)', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', height: '100%' }}>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <AreaChart data={dados} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <YAxis yAxisId="n" orientation="right" tick={{ fontFamily: 'var(--font-body)', fontSize: 10, fill: '#F59E0B' }} axisLine={false} tickLine={false} width={30} />
          <Tooltip
            ...
            {ativas.map(id => {
              ...
              return (
                <div key={id} draggable
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.3rem 0.5rem', backgroundColor: dragOver === id ? 'rgba(204,155,73,0.1)' : 'var(--color-bg-card)', border: `1px solid ${dragOver === id ? 'var(--color-ponto-conversao)' : 'var(--color-border-subtle)'}`, borderRadius: 'var(--radius-sm)', cursor: 'grab', opacity: dragId === id ? 0.4 : 1 }}>
...
export function VisaoGeral({ campanhas, estrutura, metricasDiarias, metaFalhou = false, de: deInicial, ate: ateInicial }: Props) {
  ...
  const tempData = [
    { name: 'Frio',   value: campMetricas.filter(c => c.temperatura === 'fundo').length,  color: '#60A5FA' },
    { name: 'Quente', value: campMetricas.filter(c => c.temperatura === 'quente').length, color: '#F59E0B' },
    { name: 'Não Identificado', value: campMetricas.filter(c => c.temperatura === 'neutro').length, color: '#475569' },
  ]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: 'var(--color-bg-primary)', minHeight: '100%' }}>
      ...
      {metaFalhou && (
        <div style={{ margin: '1.25rem 2rem 0', padding: '0.7rem 1rem', borderRadius: 'var(--radius-md)', border: '1px solid rgba(239,68,68,0.45)', backgroundColor: 'rgba(239,68,68,0.08)', fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#FCA5A5', fontWeight: 600 }}>
