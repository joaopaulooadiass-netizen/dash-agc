...
function BubbleMap({ dados, metrica, modo, tooltip, onHover }: BubbleMapProps) {
  ...
  function getFill(sigla: string) {
    ...
    return `rgba(204,155,73,${opacity.toFixed(2)})`
  }
          ...
          geographies.map(geo => {
            ...
            return (
              <Geography
                ...
                geography={geo}
                fill={getFill(sigla)}
                stroke={isHovered ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.12)'}
...
const METRIC_VISUAL: Record<string, { cor: string; eixo: 'L' | 'R' }> = {
  investimento: { cor: '#60A5FA', eixo: 'L' },
  valorGerado:  { cor: '#22C55E', eixo: 'L' },
  roas:         { cor: '#F59E0B', eixo: 'R' },
  cac:          { cor: '#EF4444', eixo: 'R' },
  ctr:          { cor: '#A78BFA', eixo: 'R' },
  cpm:          { cor: '#FB923C', eixo: 'R' },
  compras:      { cor: '#10B981', eixo: 'R' },
}
...
function heatmapCor(val: number, min: number, max: number, invertido?: boolean): string {
  if (max === min) return 'rgba(204,155,73,0.15)'
  const ratio = invertido
    ? 1 - (val - min) / (max - min)
    : (val - min) / (max - min)
  const clamped = Math.max(0, Math.min(1, ratio))
  return `rgba(204,155,73,${0.08 + clamped * 0.82})`
}
            ...
            {grupo(
              <>
                {([
                  ...
                  <button key={t.v} onClick={() => setTemp(t.v)} style={{ ...segBtn(temperatura === t.v, '#1e40af'), borderRight: i < arr.length - 1 ? '1px solid var(--color-border-subtle)' : 'none' }}>
                    {t.l}
                  </button>
                ))}
              </>,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
            ...
            {(() => {
              ...
              return (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '1rem' }}>
                  ...
                  <div style={{ flex: 1, height: '8px', borderRadius: '4px', background: 'linear-gradient(to right, rgba(204,155,73,0.08), rgba(204,155,73,1))' }} />
          ...
          <div style={{ backgroundColor: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-lg)', padding: '1.25rem', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
              ...
              <div style={{ flex: 1, height: '5px', borderRadius: '3px', background: 'linear-gradient(to right, rgba(204,155,73,0.15), rgba(204,155,73,0.9))' }} />
