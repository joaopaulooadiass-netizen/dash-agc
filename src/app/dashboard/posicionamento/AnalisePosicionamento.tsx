...
const PLAT_COR: Record<string, string> = {
  ...
  audience_network: '#60A5FA', messenger: '#00B2FF',
  threads: '#8B5CF6', whatsapp: '#25D366',
}
const SO_COR: Record<string, string> = {
  iOS: '#60A5FA', Android: '#34D399', Desktop: '#F59E0B', Outro: '#94A3B8',
}
const DEV_COR: Record<string, string> = {
  iphone: '#60A5FA', ipad: '#93C5FD',
  android_smartphone: '#34D399', android_tablet: '#6EE7B7',
  desktop: '#F59E0B', other: '#94A3B8',
}
// Cores vizinhas precisam contrastar — laranja (#FB923C) ao lado do âmbar
// (#F59E0B) tornava Reels FB e Stories FB indistinguíveis no gráfico
const PALETA = ['#CC9B49', '#60A5FA', '#34D399', '#F59E0B', '#A78BFA', '#22D3EE', '#EC4899', '#94A3B8']
              ...
              {filtradas.filter(m => m.grupo === grupo).map(m => {
                ...
                return (
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '0.4rem 0.55rem', marginBottom: '2px', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', backgroundColor: ativa ? 'rgba(204,155,73,0.14)' : 'transparent', color: ativa ? 'var(--color-ponto-conversao)' : 'var(--color-text-secondary)', fontFamily: 'var(--font-body)', fontSize: '0.76rem', fontWeight: ativa ? 700 : 400, textAlign: 'left' }}
                  >
                    {m.label}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontFamily: 'var(--font-body)', fontSize: '0.74rem', color: 'var(--color-text-muted)' }}>
          ...
          {AGRUPS.map(([k, l]) => (
            <button key={k} onClick={() => { setAgrup(k); setVisOverride({}) }}
              style={{ padding: '0.32rem 0.7rem', fontFamily: 'var(--font-body)', fontSize: '0.74rem', borderRadius: 'var(--radius-sm)', border: '1px solid', borderColor: agrup === k ? 'var(--color-ponto-conversao)' : 'var(--color-border-subtle)', cursor: 'pointer', backgroundColor: agrup === k ? 'rgba(204,155,73,0.14)' : 'var(--color-bg-tertiary)', color: agrup === k ? 'var(--color-ponto-conversao)' : 'var(--color-text-secondary)', fontWeight: agrup === k ? 700 : 400 }}>
              {l}
            </button>
          ))}
        </div>
            ...
            <div style={{ backgroundColor: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-lg)', padding: '1.25rem', minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {!porSemana && (
                    <button
                      ...
                      style={{ padding: '0.22rem 0.6rem', fontFamily: 'var(--font-body)', fontSize: '0.68rem', borderRadius: 'var(--radius-sm)', border: '1px solid', borderColor: suavizar ? 'var(--color-ponto-conversao)' : 'var(--color-border-subtle)', cursor: 'pointer', backgroundColor: suavizar ? 'rgba(204,155,73,0.14)' : 'var(--color-bg-tertiary)', color: suavizar ? 'var(--color-ponto-conversao)' : 'var(--color-text-secondary)', fontWeight: suavizar ? 700 : 400 }}>
                      Média 7d
                    </button>
                  )}
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={serieFinal} margin={{ top: 5, right: 8, bottom: 0, left: 0 }}>
                  ...
                  <Tooltip content={<LinhaTooltip titulo={tituloLinha} formatVal={formatLinha} labelPorChave={labelPorChave} />} cursor={{ stroke: 'rgba(255,255,255,0.15)' }} />
