    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {items.map((item, i) => {
        ...
        const cor =
          ...
          item.tipo === 'negativo'  ? 'rgba(239,68,68,0.75)' :
          'rgba(34,197,94,0.75)'
...
const CORES_PIE = ['#CC9B49', '#EF4444', '#F97316', '#F59E0B', '#22C55E', '#3B82F6', '#8B5CF6', '#EC4899']
...
export function TelaFaturamento({ metricas, metaFalhou = false, configInicial, de: deInicial, ate: ateInicial }: Props) {
  ...
  const pieDados = (() => {
    ...
    return resto > 0 ? [...top, { name: `Outros (${utmDados.length - 7})`, value: resto, cor: 'rgba(148,163,184,0.45)' }] : top
  })()
  ...
  return (
    <div style={S.page}>
      ...
      {metaFalhou && (
        <div style={{ padding: '0.7rem 1rem', borderRadius: 'var(--radius-md)', border: '1px solid rgba(239,68,68,0.45)', backgroundColor: 'rgba(239,68,68,0.08)', fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#FCA5A5', fontWeight: 600 }}>
      ...
      <div style={S.header}>
        ...
        <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          />
          {temImportacao && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', padding: '0.3rem 0.65rem', borderRadius: '999px', border: '1px solid rgba(34,197,94,0.4)', backgroundColor: 'rgba(34,197,94,0.08)', fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: 'var(--color-signal-green)', fontWeight: 600 }}>
        ...
        <div style={S.card}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={receitaDia} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="gasto"    stroke="#EF4444" strokeWidth={2} dot={false} strokeDasharray="4 2" />
            </LineChart>
          </ResponsiveContainer>
          <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', justifyContent: 'center' }}>
            {[['#22C55E', 'Receita'], ['#EF4444', 'Ads']].map(([cor, label]) => (
                ...
                {utmDados.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td style={{ padding: '0.45rem 0.5rem', textAlign: 'right' }}>
                      <span style={{ backgroundColor: 'rgba(204,155,73,0.15)', color: 'var(--color-ponto-conversao)', padding: '0.1rem 0.4rem', borderRadius: '3px', fontWeight: 600 }}>
      ...
      {/* Aviso gateway */}
      <div style={{
        backgroundColor: 'rgba(204,155,73,0.07)',
        border: '1px solid rgba(204,155,73,0.2)',
        borderRadius: 'var(--radius-md)',
